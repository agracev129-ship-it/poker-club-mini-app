    ...
    try {
        ...
        if (!existingUser) {
            ...
            existingUser = await API.createUser({
                ...
                gameNickname: user.first_name || 'Игрок',
                preferredGame: 'texas'
            });
        }

        appData.currentUser = existingUser;
        // Проверяем права админа: либо роль в БД, либо ваш ID, либо nickname admin
        appData.isAdmin = existingUser.role === 'admin' || 
                         existingUser.telegram_id === '609464085' || 
                         existingUser.gameNickname === 'admin';
        
        console.log('✅ Пользователь:', existingUser.gameNickname || existingUser.game_nickname, 'Admin:', appData.isAdmin);
